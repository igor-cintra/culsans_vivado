...
do begin                            \
  ...
  //ESCUTA .data_gnt
  always @(posedge req_ports_o[0].data_gnt) begin
    $display("[%0t] HARDWARE: req_ports_o[0].data_gnt pulsou em nivel alto", $time);
  end
  ...
  task automatic genRdReq();
    ...
    `WAIT_DIRECT(clk_i, req_ports_o[active_port].data_gnt)
  ...
  task automatic genSnoopReq(
    ...
    `WAIT_DIRECT(clk_i, snoop_port_o.ac_ready)
  ...
  task automatic cacheFilling();
    for (int i = 0; i < 2048; i++) begin
      ...
      `WAIT_DIRECT(clk_i, req_ports_o[0].data_gnt)
      req_ports_i[0].data_req  = 1'b0;
      req_ports_i[0].tag_valid     = 1'b1;
      `WAIT_CYC(clk_i,1)
      `WAIT_DIRECT(clk_i, req_ports_o[0].data_rvalid)
      req_ports_i = '0;
      `WAIT_CYC(clk_i,1)
    end
  endtask
  ...
  initial begin
    ...
    `WAIT_CYC(clk_i,1)
    `WAIT_DIRECT(clk_i,rst_ni)

    // fill the cache
    cacheFilling();

    `WAIT_DIRECT(clk_i,init_cache_done)
  ...
  initial begin
    ...
    for (int i = 0; i < 2048; i++) begin
      ...
      `WAIT_DIRECT(clk_i, axi_data_i.r.last)
    ...
    for (int i = 0; i < 2048; i++) begin
      fork
        begin
          ...
          `WAIT_DIRECT(clk_i, snoop_port_i.ac_valid)
          ...
          fork
            begin
              case({snoop_type, dirty, shared})
                {CLEAN_INVALID, 1'b0, 1'b0} : begin
                  ...
                  `WAIT_DIRECT(clk_i, snoop_port_o.cr_valid)
                ...
                {CLEAN_INVALID, 1'b0, 1'b1} : begin
                  ...
                  fork
                    ...
                    begin
                      `WAIT_DIRECT(clk_i, {axi_data_o.ar_valid & axi_data_i.ar_ready})
                ...
                {READ_UNIQUE, 1'b0, 1'b0} : begin
                  ...
                  if (!snoop_port_o.cr_resp.passDirty)
                    $error("Unexpected CR.RESP.passDirty = 0");
                  `WAIT_DIRECT(clk_i, snoop_port_o.cd_valid)
                end
                {READ_UNIQUE, 1'b0, 1'b1} : begin
                  fork
                    begin
                      `WAIT_DIRECT(clk_i, axi_data_o.ar_valid)
                      if (!isCleanUnique(axi_data_o))
                        $error("CleanUnique expected");
                    end
                    begin
            ...
            begin
              $display("[%0t] CHECKER: Iniciando a espera pelo data_gnt", $time);
              `WAIT_DIRECT(clk_i, req_ports_o[0].data_gnt)
              $display("[%0t] CHECKER: data_gnt capturado com sucesso", $time);
            end
          join
          `WAIT_CYC(clk_i,5)
          check_done = 1'b1;
