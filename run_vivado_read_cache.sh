...
CVA6_MODS=$(find $CVA6_CORE -name "*.sv" ! -name "*_pkg.sv" ! -path "*/tb/*" ! -name "riscv.sv" ! -name "std_no_dcache.sv")
...
TC_SRAM_MOD="$CVA6_DIR/vendor/pulp-platform/tech_cells_generic/src/rtl/tc_sram.sv"
...
echo "=== CACHE FILES ==="
printf "%s\n" $CVA6_MODS | grep cache_subsystem
echo "==================="
xvlog -work work -sv -d TB_HAS_LLC=0 \
-i $CVA6_CORE/include \
...
xelab -debug typical -O0 -mt off -v 1 --timescale 1ns/1ps -top culsans_tb -snapshot work_snapshot
